...
import com.ly.wanandroid.page.home.model.ArticleWrapper
import java.lang.Exception

class IndexPagingSource : PagingSource<Int, ArticleWrapper>() {
    override fun getRefreshKey(state: PagingState<Int, ArticleWrapper>): Int? =
        state.anchorPosition?.let { position ->
            val anchorPage = state.closestPageToPosition(position)
            anchorPage?.prevKey?.plus(1) ?: anchorPage?.nextKey?.minus(1)
        }

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ArticleWrapper> {
        return try {
            ...
            LoadResult.Page(
                data = (response.data?.datas ?: emptyList()).toMutableList().map {
                    ArticleWrapper(nextPageNumber, it)
                },
                prevKey = null,
